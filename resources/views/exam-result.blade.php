...
@section('title', 'Hasil Ujian - ' . $result->exam->judul)
@section('content')
    <div class="flex flex-col items-center justify-center gap-6 text-center">
        <div class="space-y-3">
            <p class="text-xs font-semibold uppercase tracking-widest text-emerald-500">Hasil Ujian</p>
            <h1 class="text-3xl font-bold text-slate-900">{{ $result->exam->judul }}</h1>
            <p class="text-sm text-slate-500">Terima kasih telah menyelesaikan ujian ini. Berikut skor yang Anda peroleh.
            </p>
        </div>
        ...
        <div class="flex h-48 w-48 items-center justify-center rounded-full border-8 border-emerald-100 bg-emerald-500/10 shadow-lg shadow-emerald-200">
            <span class="text-5xl font-bold text-emerald-600">{{ $result->nilai }}</span>
        </div>

        <a href="{{ route('my-courses.show', $result->exam->pelatihan_id) }}#tab-ujian"
