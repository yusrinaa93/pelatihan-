                    ...
                    <a href="{{ route('guest.courses') }}" class="block h-full">
                        <article class="flex flex-col overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-md hover:shadow-xl transition-all duration-300 h-full group">
                            ...
                            <div class="relative h-48 overflow-hidden bg-gradient-to-br from-emerald-50 to-emerald-100">
                                <img src="{{ $cover }}" alt="Gambar {{ $course->title }}" class="h-full w-full object-cover transition-transform duration-300 group-hover:scale-110">
                            </div>
                            {{-- Content --}}
                                ...
                                <div class="text-xs">
                                    @if($course->end_date)
                                        <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 font-semibold {{ $course->registration_ended ? 'bg-rose-100 text-rose-700' : 'bg-emerald-50 text-emerald-700' }}">
                                            ...
                                            {{ $course->registration_ended ? 'Pendaftaran berakhir' : 'Batas daftar: ' . $course->end_date->translatedFormat('d F Y') }}
                                        </span>
                                    @else
